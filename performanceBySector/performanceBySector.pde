...
void setInfoByCategory(int categoryToSet)
{
  ...
  for (String category : categoryToSet == 1 ? weightPerSector.keySet() : weightPerCountry.keySet())
  {
    ...
    {
      smallCategoryWeights.put(category, categoryToSet == 1 ? 
        weightPerSector.get(category) : weightPerCountry.get(category));
      smallCategoryHues.put(category, hueVal);
    }
    else
    {
      List<PerformanceLot> lots = categoryToSet == 1 ? 
        lotsBySector.get(category) : lotsByCountry.get(category);
      float weight = categoryToSet == 1 ? weightPerSector.get(category) * PANELWIDTH : 
        weightPerCountry.get(category) * PANELWIDTH;
      int categoryxside = (int)weight;
      setLotLocationAndColor(lots, categoryxside, weight, 
        hueVal, 0, PANELHEIGHT, categoryToSet, true);
    }
...
private void setLotLocationAndColor(List<PerformanceLot> lots, int categoryxside, 
  float weight, int hueVal, int ystart, int ytotal, int categoryToSet, boolean incrementx)
{ 
  if (weight > 100)
  {
    ...
    setLotLocationAndColor(newColumnLots, newColumnXSide, newColumnWeight, 
      hueVal, ystart, ytotal, categoryToSet, true);
    setLotLocationAndColor(remainingLots, categoryxside - newColumnXSide, weight, 
      hueVal, ystart, ytotal, categoryToSet, true);
    return;
  }
...
private void setSmallCategoryInfo(Map<String, Float> weights, 
  Map<String, Integer> hues, int categoryToSet)
{
  float totalWeight = 0; 
...
private void splitOutNewColumn(Map<String, Float> weights, 
  Map<String, Integer> hues, int categoryToSet)
{
...
private void setNewColumnInfo(Map<String, Float> weights, Map<String, Integer> hues, 
  float totalWeight, int categoryToSet)
{
  int yloc = 0;
  ...
  for (String category : weights.keySet())
  {
    ...
    setLotLocationAndColor(lots, xside, weight, hues.get(category), 
      yloc, yside, categoryToSet, false);
    yloc += yside;
  }
  xloc += xside;
}
